...
└── GroupHubContainer (React, "use client")
    ...
    │   ├── ChildrenTile (Statystyki wszystkich dzieci w grupie)
...
- **Warianty:**
    ...
    - **Dzieci:** Link do `/children`, pokazuje całkowitą liczbę dzieci w grupie.
    - **Członkowie:** Link do `/members`, pokazuje łączną liczbę osób.

## 5. Integracja API

- **Endpoint:** `GET /api/groups/:groupId`
- **Response:** `GroupDetailDTO` zawierający:
    - Podstawowe info o grupie i Twojej roli.
    - Dane admina (adminName).
    - Najbliższe wydarzenie (EventListItemDTO).
    - Statystyki (childrenCount, memberCount, upcomingEventsCount).
